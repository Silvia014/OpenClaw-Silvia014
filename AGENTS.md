...
## Silvia-Specific Working Rules

- Check `git status` before making significant repository changes.
- Inspect existing project structure before creating new files or systems.
...
- Do not expose credentials, API keys, OAuth tokens, passwords, or private configuration.
- Treat the contents of Silvia's connected accounts and workspace as private.
- When using Zapier-connected services, verify the intended action and destination before performing external actions.
- Do not send external messages or create public content without appropriate confirmation when the intent is unclear.
- When an operation fails, report the actual error instead of assuming it worked.
